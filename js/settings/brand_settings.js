...
function renderBrandSettingsToModal() {
  ...
  listContainer.innerHTML = brands.map((b, i) => `
    <div class="brand_setting_item" draggable="true" ondragstart="handleBrandDragStart(event)" ondragover="handleBrandDragOver(event)" ondragenter="handleBrandDragEnter(event)" ondragleave="handleBrandDragLeave(event)" ondrop="handleBrandDrop(event)" ondragend="handleBrandDragEnd(event)" style="background:#fff; border-radius:14px; border:1.5px solid #e2e8f0; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.05); cursor: grab;">
      <!-- Header bar -->
      <div style="display:flex; align-items:center; justify-content:space-between; padding:0.9rem 1.4rem; background:linear-gradient(135deg,#f8fafc,#f1f5f9); border-bottom:1px solid #e2e8f0;">
        <div style="display:flex;align-items:center;gap:0.8rem;">
          <div style="color:#94a3b8; font-size: 1.2rem;"><i class="fa-solid fa-grip-vertical"></i></div>
          <span class="brand_index_badge" style="background:#e2e8f0; color:#64748b; font-size:1rem; font-weight:700; padding:0.2rem 0.7rem; border-radius:20px;">#${i + 1}</span>
          <span style="font-weight:700; color:#1e293b; font-size:1.3rem;" class="brand_label_preview">${b.name || 'Brand mới'}</span>
        </div>
        <div style="display:flex; gap:0.5rem; align-items:center;">
          <button onclick="window.moveBrand(this, -1)" style="background:#f1f5f9; color:#64748b; border:1px solid #e2e8f0; width:3rem; height:3rem; border-radius:8px; cursor:pointer; display:flex; align-items:center; justify-content:center;" title="Lên trên"><i class="fa-solid fa-arrow-up"></i></button>
          <button onclick="window.moveBrand(this, 1)" style="background:#f1f5f9; color:#64748b; border:1px solid #e2e8f0; width:3rem; height:3rem; border-radius:8px; cursor:pointer; display:flex; align-items:center; justify-content:center;" title="Xuống dưới"><i class="fa-solid fa-arrow-down"></i></button>
          <button onclick="this.closest('.brand_setting_item').remove(); updateBrandIndexes();" style="background:#fee2e2; color:#ef4444; border:none; width:3rem; height:3rem; border-radius:8px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:1.2rem; transition:background .2s;" onmouseover="this.style.background='#fecaca'" onmouseout="this.style.background='#fee2e2'">
...
function addNewBrandSetting() {
  ...
  div.draggable = true;
  div.addEventListener('dragstart', window.handleBrandDragStart);
  div.addEventListener('dragover', window.handleBrandDragOver);
  div.addEventListener('dragenter', window.handleBrandDragEnter);
  div.addEventListener('dragleave', window.handleBrandDragLeave);
  div.addEventListener('drop', window.handleBrandDrop);
  div.addEventListener('dragend', window.handleBrandDragEnd);
  div.style.cssText = `background:${UI_WHITE}; border-radius:14px; border:1.5px solid ${UI_AMBER_BORDER}; overflow:hidden; box-shadow:0 2px 8px ${UI_AMBER_SOFT}; cursor: grab;`;
  div.innerHTML = `
    <div style="display:flex; align-items:center; justify-content:space-between; padding:0.9rem 1.4rem; background:linear-gradient(135deg,#fffbeb,#fef3c7); border-bottom:1px solid #fde68a;">
      <div style="display:flex;align-items:center;gap:0.8rem;">
        <div style="color:#d97706; font-size: 1.2rem;"><i class="fa-solid fa-grip-vertical"></i></div>
        <span class="brand_index_badge" style="background:#fde68a; color:#92400e; font-size:1rem; font-weight:700; padding:0.2rem 0.7rem; border-radius:20px;">Mới</span>
        <span style="font-weight:700; color:#1e293b; font-size:1.3rem;" class="brand_label_preview">Brand mới</span>
      </div>
      <div style="display:flex; gap:0.5rem; align-items:center;">
        <button onclick="window.moveBrand(this, -1)" style="background:#f1f5f9; color:#64748b; border:1px solid #e2e8f0; width:3rem; height:3rem; border-radius:8px; cursor:pointer; display:flex; align-items:center; justify-content:center;" title="Lên trên"><i class="fa-solid fa-arrow-up"></i></button>
        <button onclick="window.moveBrand(this, 1)" style="background:#f1f5f9; color:#64748b; border:1px solid #e2e8f0; width:3rem; height:3rem; border-radius:8px; cursor:pointer; display:flex; align-items:center; justify-content:center;" title="Xuống dưới"><i class="fa-solid fa-arrow-down"></i></button>
        <button onclick="this.closest('.brand_setting_item').remove(); updateBrandIndexes();" style="background:#fee2e2; color:#ef4444; border:none; width:3rem; height:3rem; border-radius:8px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:1.2rem;">
          <i class="fa-solid fa-trash"></i>
        </button>
      </div>
    </div>
...
window.moveBrand = function (btn, direction) {
  const item = btn.closest('.brand_setting_item');
  if (!item) return;
...
// Drag & Drop handlers
let dragBrandSrcEl = null;

window.handleBrandDragStart = function(e) {
  dragBrandSrcEl = e.currentTarget;
  e.dataTransfer.effectAllowed = 'move';
  e.dataTransfer.setData('text/html', dragBrandSrcEl.innerHTML);
  dragBrandSrcEl.style.opacity = '0.5';
};

window.handleBrandDragOver = function(e) {
  e.preventDefault();
  e.dataTransfer.dropEffect = 'move';
  return false;
};

window.handleBrandDragEnter = function(e) {
  e.currentTarget.style.border = '2px dashed #f59e0b';
};

window.handleBrandDragLeave = function(e) {
  e.currentTarget.style.border = '1.5px solid #e2e8f0';
};

window.handleBrandDrop = function(e) {
  e.stopPropagation();
  const target = e.currentTarget;
  target.style.border = '1.5px solid #e2e8f0';

  if (dragBrandSrcEl !== target) {
    const list = document.getElementById("brand_settings_list");
    const items = Array.from(list.children);
    const srcIndex = items.indexOf(dragBrandSrcEl);
    const targetIndex = items.indexOf(target);
    
    if (srcIndex < targetIndex) {
      target.parentNode.insertBefore(dragBrandSrcEl, target.nextSibling);
    } else {
      target.parentNode.insertBefore(dragBrandSrcEl, target);
    }
...
window.handleBrandDragEnd = function(e) {
  e.currentTarget.style.opacity = '1';
  document.querySelectorAll('.brand_setting_item').forEach(item => {
    item.style.border = '1.5px solid #e2e8f0';
  });
};

window.updateBrandIndexes = function() {
  const items = document.querySelectorAll("#brand_settings_list .brand_setting_item");
  items.forEach((item, index) => {
    const badge = item.querySelector('.brand_index_badge');
