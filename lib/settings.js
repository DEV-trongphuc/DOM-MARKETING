...
window.domGetItem = function(key) {
    const slugKey = `${key}_${window.getWorkspaceSlug()}`;
    ...
    if (oldVal !== null) {
        localStorage.setItem(slugKey, oldVal);
        return oldVal;
    }
    return null;
};
...
async function _sheetPost(payload) {
    ...
    if (typeof ACCOUNT_ID !== 'undefined') {
        postPayload.account_id = ACCOUNT_ID;
    }
...
window.initSettingsSync = async function () {
    ...
    SYNC_KEYS.forEach((key) => {
        ...
        try { window.domSetItem(key, JSON.stringify(value)); } catch (_) { }
