...
async function _fetchTokenFromSheets() {
  ...
  if (!url) return null;
  try {
    const r = await fetch(`${url}?sheet=settings&_t=${Date.now()}`, { method: "GET", cache: "no-store" });
    if (!r.ok) return null;
    const j = await r.json();
...
window._resolveMetaToken = async function () {
  ...
  */
  ...
  if (META_TOKEN_STATIC && META_TOKEN_STATIC.length > 20) {
    ...
    // Token static MÀ SAI thì bắt nhập lại luôn, không lấy từ cache cũ!
    return triggerModal();
  }
  ...
  if (sheetToken) {
    ...
    // Token Sheet MÀ SAI thì bắt nhập lại luôn
    return triggerModal();
  }
