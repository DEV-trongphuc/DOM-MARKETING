...
async function _saveTokenToAPI(token) {
  ...
  try {
    const adminToken = sessionStorage.getItem('saas_admin_token');
    ...
    await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        slug: window.SAAS_ROUTER.tenant.slug,
        token: token,
        admin_email: window._currentUser?.email || window.SAAS_ROUTER.tenant.google_email
      }),
    });
  } catch (e) {
...
window._resolveMetaToken = async function () {
  ...
  const triggerModal = () => {
    ...
    _applyToken(null);
    const openModal = () => {
      if (window._currentUser && window._currentUser.role === 'viewer') {
        document.body.innerHTML = `
          <div style="display:flex; height:100vh; align-items:center; justify-content:center; background:#f8fafc; font-family:'Roboto', sans-serif;">
              <div style="text-align:center; background:#fff; padding:3rem; border-radius:16px; box-shadow:0 10px 25px rgba(0,0,0,0.05); max-width:400px;">
                  ...
                  <h2 style="color:#1e293b; margin:0 0 1rem; font-weight:700;">Hệ Thống Đang Cấu Hình</h2>
                  <p style="color:#64748b; margin:0 0 2rem;">Chờ Admin cấu hình kết nối dữ liệu Meta. Vui lòng quay lại sau.</p>
                  <button onclick="window.location.reload()" style="padding:0.8rem 1.5rem; background:#f59e0b; color:#fff; border:none; border-radius:8px; font-weight:600; cursor:pointer; width:100%;">Tải Lại Trang</button>
              </div>
          </div>
        `;
        return;
