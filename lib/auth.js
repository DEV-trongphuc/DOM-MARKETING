...
(function () {
    ...
    const SESSION_KEY = "dom_auth_v2";
    const PENDING_KEY = "dom_pending_v2";
    ...
    function _rowUser(u, isAdmin) {
        ...
        return `<div style="display:flex;align-items:center;gap:1rem;padding:1.1rem 0;border-bottom:1px solid #f1f5f9;">
          ...
          ${isAdmin ? `<div style="display:flex;gap:.3rem;">
            ...
            ${(window._usersCache || []).find(x => x.is_owner)?.email.toLowerCase() === (window._currentUser?.email || "").toLowerCase() && !isSelf && !u.is_owner && u.status === 'active' ? `<button onclick="window._transferOwner('${u.email}')" title="Chuyển quyền Owner"
              style="width:3rem;height:3rem;border-radius:.6rem;border:1.5px solid #e2e8f0;background:#fff;cursor:pointer;color:#10b981;font-size:1.1rem;"
    ...
    window._addUser = async function () {
        ...
        try {
            const res = await _saasApi('auth_update_user', { admin_email: window._currentUser?.email, target_email: email, action_type: 'add', role });
            if (!res || !res.ok) {
                if (typeof showToast === "function") showToast(`❌ ${res?.error || "Có lỗi xảy ra"}`);
                return;
            }
