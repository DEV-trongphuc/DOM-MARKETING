/**
 * auth.js â€” Google OAuth + User Access Control
...
(function () {
    ...
    // Debug logger â€” chá»‰ hiá»‡n khi URL cÃ³ ?debug=1 hoáº·c #debug
    ...
    const SESSION_TTL = 30 * 24 * 60 * 60 * 1000; // 30 ngÃ y

    // Admin máº·c Ä‘á»‹nh dá»± phÃ²ng (Super Admin)
    ...
    // â”€â”€ SAAS API â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async function _saasApi(action, payload = {}) {
        ...
        try {
            ...
            if (res.status === 403) {
                console.error("[saas auth] 403 Unauthorized detected. Forcing sign out.");
                if (typeof window._signOut === 'function') window._signOut();
                return { ok: false, error: "PhiÃªn Ä‘Äƒng nháº­p Ä‘Ã£ háº¿t háº¡n hoáº·c báº¡n Ä‘Ã£ bá»‹ thu há»“i quyá»n truy cáº­p." };
            }
    ...
    const _outBtn = () => `<button class="_btn _btn-ghost" onclick="window._signOut()"><i class="fa-solid fa-arrow-right-from-bracket"></i> ÄÄƒng xuáº¥t</button>`;

    function _showLoading() {
        _html(_card(`<div style="text-align:center;">
            <div class="_sp2"></div>
            <p style="font-family:'Inter',sans-serif;font-size:.85rem;color:#94a3b8;margin:0;">Äang kiá»ƒm tra quyá»n truy cáº­p...</p>
        </div>`));
    }
    ...
    function _showSignIn() {
        if (!CLIENT_ID) {
            _html(_card(`${_icon("fa-solid fa-triangle-exclamation", "linear-gradient(135deg,#f59e0b,#d88200)")}
                ${_title("ChÆ°a cáº¥u hÃ¬nh Google Client ID", `ThÃªm <code style="background:#fef9c3;padding:.2rem .5rem;border-radius:.4rem;color:#92400e;font-family:monospace;">GOOGLE_CLIENT_ID</code> vÃ o <b>config_env.js</b>`)}
                <button onclick="window._skipAuth()" class="_btn _btn-amber">Bá» qua (Dev mode)</button>`));
            return;
        }
        _html(_card(`
            ${_brand()}
            <div style="text-align:center;padding:1.4rem;background:#f8fafc;border:1.5px dashed #e2e8f0;border-radius:14px;margin-bottom:1.6rem;">
                <p style="font-family:'Inter',sans-serif;font-size:.88rem;color:#64748b;margin:0 0 1.2rem;">Sá»­ dá»¥ng tÃ i khoáº£n Google Ä‘Æ°á»£c cáº¥p phÃ©p Ä‘á»ƒ tiáº¿p tá»¥c</p>
            ...
            <p style="text-align:center;font-family:'Inter',sans-serif;font-size:.75rem;color:#cbd5e1;">
                ...
                XÃ¡c thá»±c báº£o máº­t qua Google OAuth 2.0
            </p>`));
        setTimeout(() => {
    ...
    function _maskedAdmin() {
        // Láº¥y email admin Ä‘áº§u tiÃªn, che pháº§n trÆ°á»›c @ : 3 kÃ½ tá»± + "..." + pháº§n cÃ²n láº¡i
    ...
    function _showPending(email) {
        ...
        // Hiá»ƒn thá»‹ thá»i gian Ä‘Ã£ chá» dá»±a vÃ o requestAt (náº¿u cÃ³)
        const pendingData = _loadPending();
        let waitHtml = '';
        if (pendingData?.requestAt) {
            try {
                // requestAt dáº¡ng vi-VN: "02/03/2026, 20:00:00"
                const reqTime = new Date(pendingData.requestAt.replace(/(\.\d+)?$/, ''));
                if (!isNaN(reqTime)) {
                    const diffMs = Date.now() - reqTime.getTime();
                    const diffMin = Math.round(diffMs / 60000);
                    const diffText = diffMin < 1 ? 'vá»«a xong' :
                        diffMin < 60 ? `${diffMin} phÃºt trÆ°á»›c` :
                            `${Math.round(diffMin / 60)} giá» trÆ°á»›c`;
                    waitHtml = `<p style="font-size:1.1rem;color:#94a3b8;margin-top:.8rem;">YÃªu cáº§u gá»­i lÃºc ${pendingData.requestAt} (${diffText})</p>`;
                }
            } catch (_) { }
        }
        ...
        _html(_card(`
            ...
            ${_title("Äang chá» phÃª duyá»‡t", `YÃªu cáº§u cá»§a <b style="color:#f59e0b;">${email}</b> Ä‘ang chá» Admin xÃ©t duyá»‡t.`)}
            ${waitHtml}
            <button class="_btn _btn-ghost" onclick="window._checkPendingNow()" style="margin-bottom:.5rem;">
              <i class="fa-solid fa-rotate-right"></i> Kiá»ƒm tra láº¡i ngay
            </button>
            ${_outBtn()}`));
        ...
        _pendingRefreshTimer = setInterval(async () => {
            ...
            if (found?.status === 'active') {
                clearInterval(_pendingRefreshTimer);
                const pic = _loadPending()?.picture || ''; // capture trÆ°á»›c khi clear
                _clearPending();
                ...
                clearInterval(_pendingRefreshTimer);
                const p = _loadPending() || { email, name: '', picture: '' }; // capture trÆ°á»›c khi clear
                _clearPending();
    ...
    window._checkPendingNow = async function () {
        ...
        if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Äang kiá»ƒm tra...'; }
        const users = await _api();
        ...
        if (found?.status === 'active') {
            ...
            if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-rotate-right"></i> Kiá»ƒm tra láº¡i ngay'; }
            if (typeof showToast === 'function') showToast('â³ Váº«n Ä‘ang chá» admin duyá»‡t...');
        } else {
            clearInterval(_pendingRefreshTimer);
            _clearPending();
    ...
    function _showDenied(email, name, pic) {
        ...
        _html(_card(`
            ...
            ${_title("KhÃ´ng cÃ³ quyá»n truy cáº­p", "TÃ i khoáº£n cá»§a báº¡n chÆ°a Ä‘Æ°á»£c cáº¥p quyá»n xem bÃ¡o cÃ¡o nÃ y.")}
            <button id="_reqBtn" class="_btn _btn-amber" onclick="window._requestAccess('${esc(email)}','${esc(name)}','${esc(pic)}')">
                <i class="fa-solid fa-paper-plane"></i> YÃªu cáº§u quyá»n truy cáº­p
            </button>
            ${_outBtn()}`));
    }
    ...
    function _showRequested() {
        ...
        _html(_card(`
            ...
            ${_title("ÄÃ£ gá»­i yÃªu cáº§u!", `YÃªu cáº§u Ä‘Ã£ Ä‘Æ°á»£c ghi nháº­n.<br><span style="font-size:.8rem;color:#94a3b8;display:block;margin-top:.4rem;">Admin <b style="color:#64748b;">${adminLabel}</b> sáº½ phÃª duyá»‡t sá»›m.</span>`)}
            ${_outBtn()}`));
    }

    // â”€â”€ Auth Actions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    async function _handleCredential(resp) {
        _dbg("Báº¯t Ä‘áº§u xá»­ lÃ½ Ä‘Äƒng nháº­p Google...");
        const p = _jwt(resp.credential);
        if (!p) { _warn("Token Google khÃ´ng há»£p lá»‡!"); return; }
        const { email, name, picture = "" } = p;
        _dbg(`Email nháº­n Ä‘Æ°á»£c: ${email}`);
        _showLoading();

        // ÄÃ£ xÃ³a bypass DEFAULT_ADMINS Ä‘á»ƒ báº¯t buá»™c gá»i auth_check láº¥y token.

        _dbg("Äang gá»i API auth_check...");
        const check = await _saasApi('auth_check', { email });
        _dbg("Pháº£n há»“i tá»« auth_check:", check);

        if (check.ok) {
            window._everyoneViewEnabled = check.is_public;
            
            if (check.status === 'active' || check.role === 'admin') {
                _dbg(`ÄÄƒng nháº­p thÃ nh cÃ´ng: Quyá»n ${check.role}. Náº¡p token vÃ o cáº¥u hÃ¬nh.`);
            ...
            // ChÆ°a cÃ³ trong DB -> Show Denied vÃ  cho nÃºt Request
            _savePending(p);
            _showDenied(email, name, picture);
        } else {
            alert("Lá»—i xÃ¡c thá»±c: " + check.error);
        }
    }
    ...
    function _grantAccess(user) {
        ...
        // ðŸ•’ Cáº­p nháº­t lastLogin má»—i láº§n truy cáº­p
        _updateLastLogin(user.email);
    }

    function _updateLastLogin(email) {
        if (email === "dev@local") return;  // ðŸ”§ Bá» qua dev skip session
        const now = new Date().toLocaleString("vi-VN");
        console.log("[auth] _updateLastLogin â†’", email, now);
        // Note: KhÃ´ng update lastLogin trÃªn saas cho viewer ná»¯a Ä‘á»ƒ Ä‘Æ¡n giáº£n
    }

    // Background fetch â€” cáº­p nháº­t cache khÃ´ng block UI
    window._usersCache = null;
    let _autoModalShown = false;  // chá»‰ auto má»Ÿ 1 láº§n sau khi admin load trang
            ...
            if (res.ok) {
                ...
                if (isAdmin && !_autoModalShown) {
                    ...
                    if (reqs.length > 0) {
                        ...
                        // Äá»£i UI sáºµn sÃ ng rá»“i má»›i má»Ÿ modal
                        setTimeout(async () => {
                            await window.openShareModal();
                            // Chuyá»ƒn sang tab YÃªu cáº§u
                            if (typeof window._stab === "function") window._stab("requests");
                        }, 800);
                    } else {
                        // KhÃ´ng cÃ³ request â†’ Ä‘Ã¡nh dáº¥u Ä‘Ã£ check Ä‘á»ƒ interval sau khÃ´ng má»Ÿ ná»¯a
                        _autoModalShown = true;
                    }
                }
            }
    ...
    function _updateShareBadge(users) {
        ...
        // Wrapper bÃªn ngoÃ i Share button (cáº§n overflow:visible)
        const shareBtn = document.getElementById("share_url_btn");
        if (!shareBtn) return;
        // Äáº£m báº£o tháº» wrapper cÃ³ overflow visible
    ...
    // Tá»± Ä‘á»™ng refresh má»—i 60 giÃ¢y Ä‘á»ƒ cáº­p nháº­t badge request
    // Removed setInterval _bgFetchUsers per user request

    window._requestAccess = async function (email, name, pic) {
        const btn = document.getElementById("_reqBtn");
        if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin"></i> Äang gá»­i...'; }
        await _saasApi('auth_request', { email, name, picture: pic });
        // LÆ°u pending session â€” nhá»› tráº¡ng thÃ¡i giá»¯a cÃ¡c láº§n load
    ...
    // â”€â”€ User chip in toolbar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        chip.addEventListener("click", (e) => {
            ...
            dd.innerHTML = `
                ...
                <div onclick="window._signOut()" style="padding:1.1rem 1.6rem;cursor:pointer;display:flex;align-items:center;gap:.8rem;font-size:1.3rem;color:#ef4444;transition:background .15s;" onmouseover="this.style.background='#fef2f2'" onmouseout="this.style.background=''">
                    <i class="fa-solid fa-arrow-right-from-bracket"></i> ÄÄƒng xuáº¥t
                </div>`;
            chip.appendChild(dd);
    ...
    window.openShareModal = async function () {
        try {
            console.log("[ShareModal] Báº¯t Ä‘áº§u má»Ÿ modal share...");
            _buildShareModal();
            const overlay = document.getElementById("_share_modal_overlay");
            if (overlay) {
                overlay.style.display = "flex";
                overlay.classList.add("active");
                console.log("[ShareModal] ÄÃ£ hiá»ƒn thá»‹ overlay.");
            } else {
                console.error("[ShareModal] Lá»–I: KhÃ´ng tÃ¬m tháº¥y _share_modal_overlay sau khi build!");
                alert("Lá»—i: KhÃ´ng thá»ƒ khá»Ÿi táº¡o giao diá»‡n Share Modal.");
            }

            // DÃ¹ng cache táº¯c thÃ¬ (náº¿u cÃ³), sau Ä‘Ã³ refresh ngáº§m
            ...
            alert("Lá»—i khÃ´ng mong muá»‘n khi má»Ÿ modal share: " + err.message);
        }
    };
    ...
    function _buildShareModal() {
        ...
        header.innerHTML = `
            <div style="display:flex;align-items:center;gap:1.2rem;">
                ...
                <div>
                    <h2 style="margin:0;font-size:1.6rem;font-weight:800;color:#1e293b;">Chia sáº» BÃ¡o CÃ¡o</h2>
                    <p style="margin:0.2rem 0 0 0;font-size:1.15rem;color:#64748b;">Quáº£n lÃ½ quyá»n truy cáº­p vÃ  link chia sáº»</p>
                </div>
            </div>
        ...
        tabShare.innerHTML = `
          ...
          ${isAdmin ? `
          <!-- Everyone View toggle â€” chá»‰ admin tháº¥y -->
          <div id="_ev_row" style="padding:1.2rem 2.4rem;background:#fffbeb;border-bottom:1.5px solid #fde68a;display:flex;align-items:center;gap:1.2rem;flex-shrink:0;position:relative;transition:opacity .2s,background .25s;">
            ...
            <div style="flex:1;">
              <p style="font-size:1.25rem;font-weight:700;color:#1e293b;margin:0;">Chia sáº» link cho táº¥t cáº£ má»i ngÆ°á»i</p>
              <p style="font-size:1.1rem;color:#92400e;margin:0;" id="_ev_desc">${evOn ? 'ðŸ”“ Báº¥t ká»³ ai cÃ³ link Ä‘á»u cÃ³ thá»ƒ xem (khÃ´ng cáº§n Ä‘Äƒng nháº­p)' : 'ðŸ”’ Chá»‰ tÃ i khoáº£n Ä‘Æ°á»£c cáº¥p quyá»n má»›i xem Ä‘Æ°á»£c'}</p>
            </div>
            <label class="_ev_toggle" title="Báº­t/táº¯t chia sáº» cÃ´ng khai">
          ...
          <div id="_share_tabs" style="display:flex;border-bottom:1.5px solid #f1f5f9;flex-shrink:0;padding:0 2.4rem;">
            <button class="_stab" data-t="members" onclick="window._stab('members')" style="padding:1.1rem 1.8rem;border:none;background:transparent;font-size:1.3rem;font-weight:600;cursor:pointer;color:#ffa900;border-bottom:2.5px solid #ffa900;">
              <i class="fa-solid fa-user-group"></i> ThÃ nh viÃªn</button>
            <button class="_stab" data-t="requests" onclick="window._stab('requests')" style="padding:1.1rem 1.8rem;border:none;background:transparent;font-size:1.3rem;font-weight:600;cursor:pointer;color:#94a3b8;border-bottom:2.5px solid transparent;">
              <i class="fa-solid fa-inbox"></i> YÃªu cáº§u <span id="_rbadge" style="display:none;background:#ef4444;color:#fff;border-radius:3rem;padding:0 .6rem;font-size:1rem;margin-left:.3rem;"></span></button>
          </div>` : ""}
          <!-- Body -->
          <div style="flex:1;overflow-y:auto;">
            <div id="_tab_members" style="padding:1.8rem 2.4rem;display:block;">
              ...
              <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:1.2rem;padding:1.4rem;margin-bottom:1.6rem;">
                <p style="font-size:1.2rem;font-weight:700;color:#475569;margin-bottom:.9rem;"><i class="fa-solid fa-user-plus" style="color:#ffa900;"></i> ThÃªm thÃ nh viÃªn</p>
                    ...
                    <div id="_role_dd_menu" style="display:none;position:absolute;top:calc(100% + .4rem);left:0;right:0;background:#fff;border-radius:1rem;border:1.5px solid #e2e8f0;box-shadow:0 8px 32px rgba(0,0,0,.13);z-index:99999;overflow:hidden;">
                      <div onclick="window._selectRole('viewer')"
                        ...
                        <div>
                          ...
                          <p style="font-size:1.05rem;color:#94a3b8;margin:0;">Chá»‰ xem bÃ¡o cÃ¡o</p>
                        </div>
                      </div>
                      ...
                      <div onclick="window._selectRole('admin')"
                        ...
                        <div>
                          ...
                          <p style="font-size:1.05rem;color:#94a3b8;margin:0;">Quáº£n lÃ½ toÃ n quyá»n</p>
                        </div>
                      </div>
                    </div>
                  </div>
    ...
    window._toggleEveryoneView = async function (enabled) {
        ...
        try {
            ...
            if (desc) desc.innerHTML = enabled
                ? 'ðŸ”“ Báº¥t ká»³ ai cÃ³ link Ä‘á»u cÃ³ thá»ƒ xem (khÃ´ng cáº§n Ä‘Äƒng nháº­p)'
                : 'ðŸ”’ Chá»‰ tÃ i khoáº£n Ä‘Æ°á»£c cáº¥p quyá»n má»›i xem Ä‘Æ°á»£c';

            if (row) row.style.background = enabled ? "#fff7e0" : "#fffbeb";

            if (typeof showToast === "function")
                showToast(enabled ? 'ðŸ”“ ÄÃ£ báº­t chia sáº» link cÃ´ng khai' : 'ðŸ”’ ÄÃ£ táº¯t chia sáº» link cÃ´ng khai');

        } catch (e) {
            if (chk) chk.checked = !enabled;
            if (typeof showToast === "function") showToast('âŒ Lá»—i lÆ°u cÃ i Ä‘áº·t: ' + e.message);
        } finally {
    ...
    window._copyLink = function () {
        ...
        if (typeof showToast === "function") showToast("âœ… ÄÃ£ copy link!");
    };
    ...
    async function _reloadUsers() {
        ...
        // Hiá»‡u á»©ng má» khi Ä‘ang táº£i láº¡i
    ...
    function _renderUsers(all) {
        ...
        if (ul) {
            ...
            ul.innerHTML = list.length ? list.map(u => _rowUser(u, isAdmin)).join("") :
                `<p style="text-align:center;color:#94a3b8;padding:2rem;font-size:1.3rem;">ChÆ°a cÃ³ thÃ nh viÃªn</p>`;
        }
        const rl = document.getElementById("_rlist");
        if (rl) {
            rl.innerHTML = reqs.length ? reqs.map(_rowReq).join("") :
                `<div style="text-align:center;padding:3rem;">
                  ...
                  <p style="font-size:1.4rem;font-weight:700;color:#10b981;margin:0;">KhÃ´ng cÃ²n yÃªu cáº§u nÃ o!</p>
                  <p style="font-size:1.2rem;color:#94a3b8;margin:.4rem 0 0;">Táº¥t cáº£ yÃªu cáº§u Ä‘Ã£ Ä‘Æ°á»£c xá»­ lÃ½.</p>
                </div>`;
        }
    }
    ...
    function _rowUser(u, isAdmin) {
        ...
        // DÃ¹ng thÃ´ng tin tháº­t cá»§a Google náº¿u lÃ  chÃ­nh mÃ¬nh (Owner)
        ...
        const loginInfo = isSelf
            ? `<span style="font-size:1rem;color:#10b981;white-space:nowrap;"><i class="fa-solid fa-circle" style="font-size:.7rem;"></i> Äang trá»±c tuyáº¿n</span>`
            : (u.lastLogin
                ...
                : `<span style="font-size:1rem;color:#cbd5e1;">ChÆ°a Ä‘Äƒng nháº­p</span>`);
        return `<div style="display:flex;align-items:center;gap:1rem;padding:1.1rem 0;border-bottom:1px solid #f1f5f9;">
          ${avatarHtml}
          <div style="flex:1;min-width:0;">
            <p style="font-size:1.25rem;font-weight:600;color:#1e293b;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${u.name || u.email}${isSelf ? ' <span style="background:#f1f5f9;color:#94a3b8;font-size:1rem;padding:.1rem .6rem;border-radius:3rem;font-weight:500;">Báº¡n</span>' : ""}</p>
          ...
          ${isAdmin ? `<div style="display:flex;gap:.3rem;">
            ${!isSelf ? `<button onclick="window._toggleRole('${u.email}','${u.role}')" title="Äá»•i role"
            ...
            <button onclick="window._removeUser('${u.email}',this)" title="XÃ³a"
    ...
    function _rowReq(u) {
        return `<div style="display:flex;align-items:center;gap:1rem;padding:1.3rem;background:#f8fafc;border-radius:1.1rem;margin-bottom:.8rem;border:1.5px solid #e2e8f0;">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <p style="font-size:1.1rem;color:#64748b;margin:0;">${u.email}</p>
            ${u.requestAt ? `<p style="font-size:1rem;color:#94a3b8;margin:0;">YÃªu cáº§u lÃºc: ${u.requestAt}</p>` : ""}</div>
          <div style="display:flex;gap:.6rem;flex-shrink:0;">
            <button onclick="window._approveUser('${u.email}',this)"
              ...
              <i class="fa-solid fa-check"></i> Duyá»‡t</button>
            <button onclick="window._rejectUser('${u.email}',this)"
              ...
              <i class="fa-solid fa-ban"></i> Tá»« chá»‘i</button>
          </div>
        </div>`;
    }
    // â”€â”€ Custom role dropdown helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Confirm Modal helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    function _confirmModal(title, body, onConfirm, opts = {}) {
        ...
        const okLabel = opts.danger ? `<i class="fa-solid fa-trash"></i> XÃ³a` : "XÃ¡c nháº­n";
        el.innerHTML = `
          <div style="background:#fff;border-radius:1.6rem;padding:2.8rem;width:min(90vw,440px);box-shadow:0 24px 64px rgba(0,0,0,.2);animation:_cmin .2s ease;">
            ...
            <div style="display:flex;gap:.8rem;justify-content:flex-end;">
              <button id="_cm_cancel" style="padding:.85rem 2rem;background:#f1f5f9;color:#64748b;border:none;border-radius:.9rem;font-size:1.3rem;font-weight:600;cursor:pointer;transition:background .15s;"
                onmouseover="this.style.background='#e2e8f0'" onmouseout="this.style.background='#f1f5f9'">Huá»·</button>
        ...
        document.getElementById("_cm_ok").addEventListener("click", async () => {
            ...
            if (okBtn) {
                okBtn.disabled = true;
                okBtn.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Äang xá»­ lÃ½...`;
    ...
    window._addUser = async function () {
        ...
        if (!email) return;
        // Loading state trÃªn nÃºt ThÃªm
        const addBtn = document.querySelector("button[onclick='window._addUser()']");
        if (addBtn) _btnLoading(addBtn, true, "Äang thÃªm...");
        try {
            ...
            await _reloadUsers();
            if (typeof showToast === "function") showToast(`âœ… ÄÃ£ thÃªm ${email}`);
        } finally {
    ...
    window._toggleRole = function (email, cur) {
        ...
        const icon = newRole === "admin" ? "ðŸ‘‘" : "ðŸ‘¤";
        _confirmModal(
            `${icon} Äá»•i role thÃ nh <b>${newRole}</b>?`,
            `<span style="color:#64748b;font-size:1.25rem;">TÃ i khoáº£n <b style="color:#1e293b;">${email}</b> sáº½ Ä‘Æ°á»£c chuyá»ƒn sang role <b style="color:${newRole === "admin" ? "#ffa900" : "#64748b"}">${newRole}</b>.</span>`,
            async () => {
                await _saasApi('auth_update_user', { admin_email: window._currentUser?.email, target_email: email, action_type: 'role', role: newRole });
                await _reloadUsers();
                if (typeof showToast === "function") showToast(`âœ… ÄÃ£ Ä‘á»•i ${email} â†’ ${newRole}`);
            }
        );
    };
    ...
    window._removeUser = function (email, btnEl) {
        _confirmModal(
            `<span style="color:#ef4444;"><i class="fa-solid fa-trash"></i> XÃ³a thÃ nh viÃªn?</span>`,
            `<div style="background:#fef2f2;border:1.5px solid #fecaca;border-radius:1rem;padding:1.2rem 1.4rem;margin-bottom:.4rem;">
              ...
              <p style="font-size:1.15rem;color:#94a3b8;margin:.3rem 0 0;">TÃ i khoáº£n nÃ y sáº½ bá»‹ xÃ³a khá»i danh sÃ¡ch truy cáº­p.</p>
            </div>`,
            async () => {
                _btnLoading(btnEl, true, "Äang xÃ³a...");
                await _saasApi('auth_update_user', { admin_email: window._currentUser?.email, target_email: email, action_type: 'remove' });
                await _reloadUsers();
                if (typeof showToast === "function") showToast(`ðŸ—‘ï¸ ÄÃ£ xÃ³a ${email}`);
            },
            { danger: true }
        );
    };
    ...
    window._approveUser = async function (email, btnEl) {
        ...
        if (btnEl) {
            btnEl.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Äang duyá»‡t...`;
        ...
        try {
            ...
            await _reloadUsers();
            if (typeof showToast === "function") showToast(`âœ… ÄÃ£ duyá»‡t ${email}`);
        } catch (e) {
            btns?.forEach(b => { b.disabled = false; b.style.opacity = "1"; b.style.cursor = "pointer"; });
            if (btnEl && origHTML) btnEl.innerHTML = origHTML;
            if (typeof showToast === "function") showToast(`âŒ Lá»—i: ${e.message}`);
        }
    };
    ...
    window._rejectUser = async function (email, btnEl) {
        ...
        const origHTML = btnEl?.innerHTML;
        if (btnEl) {
            btnEl.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Äang tá»« chá»‘i...`;
            btnEl.style.minWidth = btnEl.offsetWidth + 'px';
        }
        try {
            await _saasApi('auth_update_user', { admin_email: window._currentUser?.email, target_email: email, action_type: 'reject' });
            await _reloadUsers();
            if (typeof showToast === "function") showToast(`ðŸ—‘ï¸ ÄÃ£ tá»« chá»‘i ${email}`);
        } catch (e) {
            btns?.forEach(b => { b.disabled = false; b.style.opacity = "1"; b.style.cursor = "pointer"; });
            if (btnEl && origHTML) btnEl.innerHTML = origHTML;
            if (typeof showToast === "function") showToast(`âŒ Lá»—i: ${e.message}`);
    ...
    function _rowLoading(email, action) {
        ...
        rows.forEach(row => {
            ...
            if (target) target.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> ${action === "approve" ? "Äang duyá»‡t..." : "Äang xÃ³a..."}`;
        });
    }
    ...
    async function _boot() {
        ...
        // Pre-check: XÃ¡c minh tenant cÃ³ tá»“n táº¡i khÃ´ng trÆ°á»›c khi lÃ m báº¥t ká»³ Ä‘iá»u gÃ¬
        const slug = _getTenantSlug();
        if (slug) {
            try {
                const precheck = await _saasApi('auth_check', { email: '' });
                if (precheck.error === "Tenant not found") {
                    console.log("[AUTH] Tenant khÃ´ng tá»“n táº¡i. Dá»«ng luá»“ng auth Ä‘á»ƒ router xá»­ lÃ½ HTML Not Found.");
                    return; // Ngá»«ng auth flow ngay láº­p tá»©c
                }
                if (precheck.ok) {
                    window._everyoneViewEnabled = precheck.is_public;
                    
                    // Náº¾U LÃ€ SUPER ADMIN (hoáº·c Admin cÃ³ session há»£p lá»‡ qua token backend) => Bypass
                    if (precheck.role === 'admin') {
                        console.log("[auth] ðŸ‘‘ Super Admin detected via backend token â†’ bypass auth");
                    ...
                    if (window._everyoneViewEnabled && !window._currentUser) {
                        console.log("[auth] ðŸŒ everyone_view = ON â†’ bypass auth");
                        _applyEveryoneView();
                    }
                }
            } catch (e) {
                console.warn("[AUTH] Lá»—i khi pre-check tenant:", e);
            }
        }
        ...
        if (cached?.status === "active") {
            ...
            // Pháº£i gá»i auth_check Ä‘á»ƒ láº¥y láº¡i token vÃ¬ ban Ä‘áº§u get_tenant Ä‘Ã£ giáº¥u nÃ³ Ä‘i
            try {
                ...
                if (check.ok && (check.status === 'active' || check.role === 'admin')) {
                    ...
                    if (check.meta_token) window.APP_CONFIG.META_TOKEN = check.meta_token;
                    if (check.gemini_api_key && window.SAAS_ROUTER && window.SAAS_ROUTER.tenant) {
                        window.SAAS_ROUTER.tenant.gemini_api_key = check.gemini_api_key;
                    }
                    if (check.ad_accounts) {
                        try { window.APP_CONFIG.ALLOWED_ACCOUNTS = JSON.parse(check.ad_accounts); } 
                        catch(e) { window.APP_CONFIG.ALLOWED_ACCOUNTS = check.ad_accounts; }
                    ...
                    // Máº¥t quyá»n truy cáº­p -> xÃ³a session vÃ  yÃªu cáº§u Ä‘Äƒng nháº­p láº¡i
    ...
    function _applyEveryoneView() {
        ...
        window._currentUser = { email: "", name: "KhÃ¡ch", picture: "", role: "viewer", status: "active", isGuest: true };
        _authResolve();
    }
