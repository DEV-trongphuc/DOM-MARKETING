...
(function () {
    ...
    const _CSS = `<style>
        ...
        ._ac{background:#fff;border-radius:20px;padding:2.4rem 2.2rem;box-shadow:0 28px 70px rgba(0,0,0,.4),0 0 0 1px rgba(255,255,255,.08);animation:_acIn .38s cubic-bezier(.16,1,.3,1) both;}
        @keyframes _acIn{from{opacity:0;transform:translateY(18px) scale(.97)}to{opacity:1;transform:none}}
        ._bar{height:4px;background:linear-gradient(90deg,#f59e0b,#fbbf24 50%,#34d399);border-radius:20px 20px 0 0;margin:-2.4rem -2.2rem 2rem;}
    ...
    window.openShareModal = async function () {
        ...
        if (typeof switchSettingTab === "function") {
            switchSettingTab("tab-share", true);
        }
    ...
    function _buildShareModal() {
        ...
        tabShare.innerHTML = `
        <div style="background:#fff;display:flex;flex-direction:column;height:100%;animation:_sin .3s ease;">
          <style>
        ...
        if (arguments[0] !== true) {
            document.getElementById("filter_settings_modal").style.display = "flex";
            if (typeof switchSettingTab === 'function') switchSettingTab('tab-share', true);
        }
